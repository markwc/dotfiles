...
;;
;; $Id: verilog-mode.el 336 2007-05-31 23:33:09Z mac $
...
(defconst verilog-mode-version (substring "$$Revision: 336 $$" 12 -3)
  "Version of this verilog mode.")
(defconst verilog-mode-release-date (substring "$$Date: 2007-05-31 16:33:09 -0700 (Thu, 31 May 2007) $$" 8 -3)
  "Version of this verilog mode.")
...
(if verilog-mode-map
  ...
  (define-key verilog-mode-map "\t"       'verilog-tab)
;  (define-key verilog-mode-map "\t"       'electric-verilog-tab)
  ...
  (define-key verilog-mode-map ","	  'verilog-electric-comma)
  (define-key verilog-mode-map "."	  'verilog-electric-period)
  )
...
(defun verilog-tab (&optional pre-arg)
  ...
  (cond ((= (char-syntax (preceding-char)) ?w)
	...
	((> (current-column) (current-indentation))
	 (tab-to-tab-stop))
	((and (or (eq last-command 'verilog-tab))))
		  ;;(eq last-command 'verilog-return))
	      ;;(/= 0 (current-indentation)))
	 ;;(backward-delete-char-untabify verilog-basic-offset nil))
	((verilog-indent-line))
        )
  (setq this-command 'verilog-tab)
  )
...
;; Added electric stutter scs
;(defun verilog-electric-comma (count) "',,' --> ' <= '"
;  (interactive "p")
;  (if (and (= count 1))
...
;
;(defun verilog-electric-period (count) "'..' --> ' => '"
;  (interactive "p")
;  (if (and (= count 1))
;      (cond ((= (preceding-char) last-input-char)
;	     (progn (delete-char -1)
